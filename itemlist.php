...
<head>
  <title> Products | Zarathelle PH </title>
...
<body>
  ...
  <div class="container" id="all-items" style="width:95%;">

    <!-- Display all items from items table -->
    <?php

    require 'connection.php';
    $conn = Connect();

    $sql = "SELECT * FROM ITEMS WHERE options = 'ENABLE' ORDER BY ID";
    ...
    <div class="col-md-3">
      ...
      <form method="post" action="cart.php?action=add&id=<?php echo $row["ID"]; ?>">
        <div class="mypanel" style="text-align: center; align-items: center;" ;>
          <img src="<?php echo $row["images_path"]; ?>" class="img-responsive">
          <h4 class="text-dark"><?php echo $row["name"]; ?></h4>
          <h5 class="text-info"><?php echo $row["description"]; ?></h5>
          <h5 class="text-danger">&#8377; <?php echo $row["price"]; ?>/-</h5>
          <h5 class="text-info">
            Quantity: <input type="number" min="1" max="25" name="quantity" class="form-control " value="1"
              style="width: 60px;">
          </h5>
